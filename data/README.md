# Data
The expectations for data described in this section are specific to achieving compatibility with the code in this repo, but also represent a set expectations of that can generally be abided by if the goal is creating machine-processable glossed data.

Note that some of the scripts found in src/ and in the data repos can be helpful in the process of getting the data from its initial glossed state to being ready for machine processing.
...
It's fine if the data contains more than three lines per sentence (e.g., a translation line), but these will be just be ignored by the system.  Ensure that you specify the correct line numbers for the lines of interest when you use the code.

## Data Expectations
There are certain expectations in place about the format of data inputted to the system, so that it knows how to handle the different lines.  In preparing data for this system, I had to confront many decisons about what data should look like.  Often, there are many reasonable possibilities, but nonetheless one must be selected for the sake of consistency.  These expectations have been compiled into a list at the end of this section.
...
- However, other expectations are more general best practices for glossing, relevant outside just this project.  For example, using a special boundary to mark reduplication (~), or marking stress in the same spot across the transcription and segmentation lines.  In many such cases, different fieldworkers may have a different practice -- and indeed, it may be just as reasonable (as long as you are consistent about it!).  In order to have clean, consistent data, I simply had to make *a* choice on these matters and commit to it.
- ...and some expectations fall into both of these categories!

I have sorted the expectations below according to these categories, to make it clearer which must be *absolutely* followed to have the system handle your data, and which are just guidelines.
...
One final note is that I have seen (e.g., in Gitksan data) '???' used in the segmentation line for unknown morphemes.  Instead, I have changed these unknown morphemes to a) just the same as the transcription line in the segmentation line and b) UNK (= unknown) in the gloss line.  
